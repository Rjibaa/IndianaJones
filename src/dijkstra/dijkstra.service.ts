...
export class DijkstraService {
    ...
    createGraph(trains:Train[]){
        ...
        for (const train of trains) {
            ...
            graph[train.departureCity][train.arrivalCity] = calculateArrivalTime(train.departureTime,train.travelTime);
        }
    ...
    findShortestPath(trains: Train[], departureCity: string, arrivalCity: string, startTime: string): Dijkstra {
        ...
        while (!pq.isEmpty()) {
            ...
            if (currentNode === arrivalCity) {
                ...
                while (prevNodes[node]) {
                    const train = trains.find(
                        (train) => train.departureCity === prevNodes[node] && train.arrivalCity === node && calculateArrivalTime(train.departureTime,train.travelTime)===nodesCostFromStart[node]
                      );
                    if (train) {
            ...
            for (const neighbor in graph[currentNode]) {
                ...
                console.log(currentNode,neighbor,newCost,nodesCostFromStart[neighbor])
